...
export const AnalyticsSection = () => (
  <section className="py-12 sm:py-16 md:py-24 border-t border-[rgba(22,20,14,0.11)] bg-transparent relative">
    <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-8">
      <SectionHeading
        eyebrow="Key Benchmarks"
        title="Quality & Speed You Can Rely On."
        sub="We build every website with high standards for speed, mobile responsiveness, and quick project turnaround."
        center
      />
          ...
          <Reveal key={s.label} delay={i * 0.06}>
            <InteractiveCard className="h-full">
              ...
              <div className="p-4 bg-[#EFECE3] shadow-[inset_0_2px_4px_rgba(22,20,14,0.03)] border-b border-[rgba(22,20,14,0.11)] flex items-center justify-between">
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center border ${(ACCENT[s.accent] || ACCENT.blue).border} ${(ACCENT[s.accent] || ACCENT.blue).bg} shadow-sm`}>
                  <s.icon size={18} className={(ACCENT[s.accent] || ACCENT.blue).text} />
                </div>
      ...
      <p className="text-center text-[#6B6557] text-xs mt-7 font-mono font-medium">
        * Standard specifications included with all website builds
      </p>
    </div>
  </section>
);
