...
export const DemoSection = () => {
  ...
  return (
    <section id="demo" className="py-16 md:py-24 border-t border-slate-800/60 bg-transparent relative">
        ...
        <div className="mt-14 grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <Reveal className="lg:col-span-7 flex flex-col">
            <InteractiveCard accentColor="rgba(56, 189, 248, 0.15)" className="p-6 sm:p-7 flex flex-col justify-between h-full">
              <div>
                ...
                <div className="flex items-center justify-between mb-6 pb-3 border-b border-slate-800/80 text-xs font-mono">
                  <div className="flex items-center gap-2">
                    <span className="w-2.5 h-2.5 rounded-full bg-rose-500/70" />
                    <span className="w-2.5 h-2.5 rounded-full bg-amber-500/70" />
                    <span className="w-2.5 h-2.5 rounded-full bg-emerald-500/70" />
                    <span className="ml-2 font-mono text-slate-300">pipeline.arsak.config</span>
                  </div>
                  <div className="flex items-center gap-1.5 bg-slate-900/90 border border-slate-800 p-0.5 rounded-lg">
                    <button
                      onClick={() => setActiveTab("config")}
                      className={`px-2.5 py-1 rounded text-[11px] font-mono transition-colors ${activeTab === "config" ? "bg-slate-800 text-white font-semibold" : "text-slate-400 hover:text-slate-200"}`}
                    >
                      Config
                    </button>
                    <button
                      onClick={() => setActiveTab("metrics")}
                      className={`px-2.5 py-1 rounded text-[11px] font-mono transition-colors ${activeTab === "metrics" ? "bg-slate-800 text-white font-semibold" : "text-slate-400 hover:text-slate-200"}`}
                    >
                      Metrics
                    </button>
                  </div>
                ...
                <div className="font-mono text-sm sm:text-base leading-relaxed tracking-tight select-none mb-7 bg-slate-950/40 p-4 rounded-xl border border-slate-800/60">
                  <div className="flex flex-wrap items-center">
                    <span className="text-purple-400 font-medium">const</span>
                    <span className="text-white ml-2 font-medium">workflow</span>
                  ...
                  <div className="text-slate-400 text-xs sm:text-sm my-2 font-normal">
              ...
              <div className="rounded-xl border border-slate-800/90 bg-[#03060f] p-5 sm:p-6 mt-auto shadow-inner">
                  ...
                  {bars.map((bar) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {isHovered && (
                          <div className="absolute -top-11 bg-slate-900/95 border border-slate-700 text-white text-[11px] font-mono px-2.5 py-1 rounded shadow-xl whitespace-nowrap z-30 pointer-events-none transform -translate-y-1 transition-all">
                            <div className="font-semibold text-cyan-300">{bar.label}</div>
                            <div className="text-slate-300 text-[10px]">{bar.value} · {bar.latency}</div>
                          </div>
                        )}
                ...
                <div className="flex justify-between items-center mt-3 pt-3 border-t border-slate-900 text-[11px] font-mono text-slate-500">
          ...
          <Reveal delay={0.06} className="lg:col-span-5 flex flex-col">
            <InteractiveCard accentColor="rgba(168, 85, 247, 0.14)" className="p-6 sm:p-7 flex flex-col justify-between h-full">
              <div>
                ...
                <div className="bg-[#03060f] border border-slate-800/90 rounded-xl p-4 font-mono text-xs text-slate-300 space-y-2.5 shadow-inner">
                  <div className="flex justify-between items-center py-0.5 border-b border-slate-900/60">
                    <span className="text-slate-500">customer_entity:</span>
                    <span className="text-white font-medium">"ABC Industries Ltd."</span>
                  </div>
                  <div className="flex justify-between items-center py-0.5 border-b border-slate-900/60">
                    <span className="text-slate-500">document_id:</span>
                    <span className="text-cyan-300">"#INV-2048-HQ"</span>
                  </div>
                  <div className="flex justify-between items-center py-0.5 border-b border-slate-900/60">
                    <span className="text-slate-500">verified_items:</span>
                    <span className="text-emerald-400 font-semibold">14 Line Items (100%)</span>
                  </div>
                  <div className="flex justify-between items-center py-0.5 border-b border-slate-900/60">
                    <span className="text-slate-500">reconciliation_rate:</span>
                    <span className="text-amber-300">99.8% Confidence</span>
                  </div>
                  <div className="flex justify-between items-center py-0.5">
                    <span className="text-slate-500">destination_sync:</span>
              ...
              <div className="mt-6 pt-4 border-t border-slate-800/80 flex items-center justify-between gap-3">
                ...
                {pipelineState === "completed" && (
                  <button
                    onClick={handleReset}
                    className="p-2.5 rounded-lg border border-slate-700 bg-slate-900 hover:bg-slate-800 text-slate-300 transition-colors cursor-pointer"
                    title="Reset"
                  >
                    <RotateCcw size={15} />
                  </button>
          ...
          <Reveal delay={0.10} className="lg:col-span-4">
            <InteractiveCard accentColor="rgba(59, 130, 246, 0.15)" className="p-6 flex flex-col justify-between h-full">
              ...
              <div>
                ...
                <div className="text-sm text-slate-300 font-medium mt-1">Average Automation Rate</div>
          ...
          <Reveal delay={0.12} className="lg:col-span-4">
            <InteractiveCard accentColor="rgba(16, 185, 129, 0.14)" className="p-6 flex flex-col justify-between h-full">
              ...
              <div className="space-y-2 text-xs font-mono">
                <div className="flex items-center justify-between p-2 rounded-lg bg-[#03060f] border border-slate-800/80 hover:border-slate-700 transition-colors">
                  <span className="text-slate-300">SAP / Oracle ERP</span>
                ...
                <div className="flex items-center justify-between p-2 rounded-lg bg-[#03060f] border border-slate-800/80 hover:border-slate-700 transition-colors">
                  <span className="text-slate-300">PostgreSQL / Cloud</span>
                  <span className="text-emerald-400 flex items-center gap-1">
              ...
              <div className="mt-4 pt-3 border-t border-slate-800/60 flex items-center justify-between text-xs font-mono text-slate-400">
                ...
                <span className="text-white font-medium">42 Live Feeds</span>
              </div>
            </InteractiveCard>
          </Reveal>
          ...
          <Reveal delay={0.14} className="lg:col-span-4">
            <InteractiveCard accentColor="rgba(245, 158, 11, 0.14)" className="p-6 flex flex-col justify-between h-full">
              ...
              <div>
                <div className="text-sm font-semibold text-white mb-1.5">Autonomous Exception Routing</div>
